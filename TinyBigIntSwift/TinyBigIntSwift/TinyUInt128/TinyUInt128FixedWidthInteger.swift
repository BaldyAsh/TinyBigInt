...
//        result: {(BIT1,BIT2) (BIT2,BIT3,BIT4)} {(BIT4,BIT5,BIT6) (BIT6,BIT7)}
    ...
    internal func quotientAndRemainderFullWidth(dividingBy dividend: (high: TinyUInt128, low: TinyUInt128)) -> (quotient: TinyUInt128, remainder: TinyUInt128) {
        ...
        let divisor = self
        var numeratorLength: TinyUInt128
        ...
        for numeratorShift in (0...numeratorLength).reversed() { // for i := n − 1 .. 0 do
            remainder <<= 1 // R := R << 1
            remainder |= TinyUInt128.bitFromPosition(at: numeratorShift, for: dividend) // R(0) := N(i)
    ...
    internal static func bitFromPosition(at bitPosition: TinyUInt128, for input: (high: TinyUInt128, low: TinyUInt128)) -> TinyUInt128 {
        switch bitPosition {
        case 0:
            return input.low & 1
        case 1...127:
